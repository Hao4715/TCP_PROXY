...
void read_conf()
{
    FILE *file = fopen("test_conf.conf","r");
    int ch,i;
    int blockNum = 0;
    int proxyNum = proxy_nums;
    ...
    while((ch = fgetc(file)) != EOF)
    {
        ...
        {
            if(blockNum == proxyNum)
            {
                proxyInfo = new_proxy(proxyInfo,&proxyNum,blockNum);
            }
            block_read(file,&blockNum,&ch,proxyInfo);
        }
    }
    for(i=0;i<blockNum;i++)
    {
